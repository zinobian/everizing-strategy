...
async function getOnePrice(ticker, excd, accessToken, appKey, appSecret) {
  try {
    const data = await inquirePrice(accessToken, appKey, appSecret, ticker, excd);

    if (data.rt_cd !== '0') {
      console.error(`${ticker}(${excd}):`, data.msg1 || data);
    ...
    console.error(`${ticker} 예외:`, e.message);
    return { price: 0, prevClose: 0, change: 0, currency: 'USD', name: ticker };
  }
}
...
async function getPrices(tickers) {
  ...
  for (const ticker of tickers) {
    ...
    results[ticker] = await getOnePrice(ticker, excd, accessToken, appKey, appSecret);
